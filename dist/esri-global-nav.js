	...
	function renderSingle(_ref3) {
		...
		if (item.menus.length >= 10) {
			if (item.menus.length % 2 === 0) {
				...
				columns = $assign('div', { class: prefix$4 + '-sublist--col-wrapper' }, createMenuColumns(item.menus.slice(0, multi)), createMenuColumns(item.menus.slice(parseInt(multi), item.menus.length - 1)));
			} else if (item.menus.length % 3 === 0) {
				var _multi = item.menus.length / 3;

				columns = $assign('div', { class: prefix$4 + '-sublist--col-wrapper' }, createMenuColumns(item.menus.slice(0, _multi)), createMenuColumns(item.menus.slice(parseInt(_multi), parseInt(_multi * 2))), createMenuColumns(item.menus.slice(parseInt(_multi * 2), item.menus.length - 1)));
			}
		} else {
			if (hasMenuItems) {
				columns = $assign('div', { class: prefix$4 + '-sublist--col-wrapper' }, createMenuColumns(item.menus.slice(0, item.menus.length - 1)));
			}
		}
